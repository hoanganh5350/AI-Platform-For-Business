...
export const Step2UIFlow: React.FC<Step2UIFlowProps> = ({ form, onBack, onFinish }) => {
  return (
    <Card title="Mô tả luồng màn hình doanh nghiệp của bạn" bordered={false} style={{ boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
      ...
      >
                    ...
                    {({ getFieldValue }) => {
                      ...
                      const availableParents = blocks
                        ...
                        .map((b: UIBlockForm, i: number) => ({
                          ...
                          value: b?.name ? b.name.toLowerCase().replace(/[^a-z0-9]/g, '-') : `node-${i}`
                        }));
        ...
        <Space style={{ width: '100%', justifyContent: 'space-between' }}>
          ...
          <Button type="primary" onClick={onFinish}>Hoàn thành cấu hình</Button>
        </Space>
      </Form>
    </Card>
  );
